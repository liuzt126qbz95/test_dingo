...
def run_dwaCore(map, map_width, map_height, map_origin_X, map_origin_Y, map_resolution, reference_XYThetas, cur_X, cur_Y, cur_Theta, cur_v_x, cur_v_y, cur_w_z):
    ...
    score_dev_set, score_fast_set, score_heading_set, score_dist_set, score_set = np.array([]), np.array([]), np.array([]), np.array([]), np.array([])
    ...
    for idx in range(len(trajectory_set)):
        ...
        # near_goal
            ...
            score += W_HEADING*(score_heading_set[idx]/np.sum(score_heading_set))*(2)
            score += W_DIST*(score_dist_set[idx]/np.sum(score_dist_set))
...
def get_simulated_trajectory_and_safety_score(cur_X, cur_Y, cur_Theta, cur_v_x, cur_v_y, cur_w_z, control_v_x, control_v_y, control_w_z, map, map_width, map_height, map_origin_X, map_origin_Y, map_resolution, prediction_time):
    '''
    ...
    for i in range(n_steps):
        ...
        for XY in get_corners(cur_X, cur_Y, cur_Theta):
            ...
            if not is_node_free_and_in_map_range_loose(cmap_X, cmap_Y, map, map_width, map_height):
                return [], 0
