...
from model_kinematic_diff import *
from utils import *



INF = 999999999.
MIN_V_X = -1 # [m/s] default 1.3
MAX_V_X = 1 # [m/s]
MIN_V_Y = -1
MAX_V_Y = 1
MIN_W_Z = -1 # [radians]
...
def get_tracking_point_and_tracking_trajectory(cur_X, cur_Y, L, reference_trajectory):
    ...
    for idx in range(closest_idx, len(reference_trajectory)):
        ...
        if hypot(Y-cur_Y, X-cur_X) > L:
            return [X, Y, Theta], reference_trajectory[idx:]
    return reference_trajectory[-1][0:3], reference_trajectory[closest_idx:]


def get_simulated_trajectory_and_check_collision(cur_X, cur_Y, cur_Theta, cur_v_x, cur_v_y, cur_w_z, control_v_x, control_v_y, control_w_z, map, map_width, map_height, map_origin_X, map_origin_Y, map_resolution):
    '''
...
def score_trajectory_by_fast(trajectory):
    ...
    速度评价
    '''
    average_speed = abs(np.average(trajectory[1:,3]))
