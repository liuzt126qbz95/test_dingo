...
# planner setting
PLANNER = 1         # 1:2-step jps      2:bidirectional-astar    3:astar     0:presetted path
SMOOTHER = 1        # 1:ON              2:OFF
CONTROLLER = 1      # 1:DDWA            2:DWA
    ...
    def __init__(self):
        ...
        while not rospy.is_shutdown():
            ...
            if PLANNER == 0:
                ...
                node_XYs = run_aStarBiCore(self.gcmap, self.gcmap_width, self.gcmap_height, self.start_X, self.start_Y, self.goal_X, self.goal_Y)
            elif PLANNER == 3:
                node_XYs  = run_aStarCore(self.gcmap, self.gcmap_width, self.gcmap_height, self.start_X, self.start_Y, self.goal_X, self.goal_Y)
            else:
                print("planner not selected...")
            ...
            if node_XYs == None:
                ...
                if CONTROLLER == 1: 
                    ...
                    if trajectory == None:
                        control, trajectory, tracking_point, driving_status = run_dwaCore(self.gcmap, self.gcmap_width, self.gcmap_height, self.gcmap_origin_X, self.gcmap_origin_Y, self.gcmap_resolution, 
                                                                        np.array(path_XYThetas), 
                                                                        self.cur_X, self.cur_Y, self.cur_Theta, self.cur_v_x, self.cur_v_y, self.cur_w_z,
                                                                        enable_drift=False,
                ...
                # 发布navigation status
                string_msg = String()
                string_msg.data = driving_status
                self.status_pub.publish(string_msg)
